  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-center">
        ...
        <span className="font-mono text-cyan-400/70 text-xs">{level}%</span>
      </div>
      <div className="h-1.5 bg-[#0c1a35] rounded-full overflow-hidden">
        <div
          ...
          style={{ width: `${level}%`, boxShadow: '0 0 8px rgba(34,211,238,0.4)' }}
        />
      </div>
    </div>
  )
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-14">
          {skillGroups.map((group) => (
            <div
              key={group.category}
              className="reveal bg-[#050e1f] border border-cyan-400/10 rounded-lg p-6 card-hover"
            >
              <div className="flex items-center gap-3 mb-6">
                <span className="text-xl">{group.icon}</span>
                <h3 className="font-display font-semibold text-white ls-wide uppercase tracking-widest text-sm">{group.category}</h3>
              </div>
